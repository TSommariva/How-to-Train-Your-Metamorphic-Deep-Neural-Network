...
def main(args):
    ...
    iterative_steps = 5
    data = defaultdict(lambda: {'pruning_ratio': [], 'accuracy': []})
    ...
    for i, (method, values) in enumerate(data.items()):
        ...
        plt.plot(pruning_ratios, accuracies, marker=markers[i % len(markers)], label=method)
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(f'pruning_{random_value}.png', dpi=300, bbox_inches='tight')
    print(f'pruning_{random_value}.png')
